...
use crate::{
    ...
    pad::Padded,
    chain::{Chain, IntoChain, Single, Nothing, Optional},
    parse_fn::{
        ParseFn, RcParseFn, GenParseFn,
        ThenFn, AlsoFn, ChainFn,
        OrFn, OrFallbackFn, OrChainFallbackFn,
    },
};
...
pub struct Parser<'a, T, O, E = DefaultParseError<T>, F = RcParseFn<'a, T, O, E>>
    where
        T: Clone + 'a,
        O: Chain + 'a,
        E: ParseError<T> + 'a,
...
impl<'a, T: Clone + 'a, O: Chain + 'a, E: ParseError<T> + 'a, F: ParseFn<T, O, E> + 'a> Clone for Parser<'a, T, O, E, F> {
    fn clone(&self) -> Self {
        Self {
            f: self.f.clone(),
...
impl<'a, T: Clone + 'a, O: Chain + 'a, E: ParseError<T> + 'a> Parser<'a, T, O, E> {
    ...
    fn try_map<'b, U>(f: impl Fn(T) -> Result<U, E> + Clone + 'a) -> Parser<'b, T, Single<U>, E, impl ParseFn<T, Single<U>, E> + Captures<'b> + 'a>
        where 'a: 'b, 'b: 'a
    {
        Parser::generic_raw(move |tokens: &mut TokenIter<T>| try_parse(tokens, |(idx, tok), _| {
            match f(tok.clone()) {
                Ok(output) => Ok((MayFail::none(), Single::from(output))),
...
impl<'a, T: Clone + 'a, O: Chain + 'a, E: ParseError<T> + 'a, F: ParseFn<T, O, E> + 'a> Parser<'a, T, O, E, F> {
    fn new(f: F) -> Self {
        Self {
            f,
            _phantom: PhantomData,
    ...
    pub fn map<'b, U: 'a>(self, f: impl Fn(O) -> U + Clone + 'a) -> Parser<'a, T, Single<U>, E, impl ParseFn<T, Single<U>, E> + Captures<'b> + 'a>
        where 'a: 'b, 'b: 'a
    {
        Parser::generic_raw(move |tokens| self.f.parse(tokens).map(|(e, o)| (e, f(o).into())))
    }

    pub fn map_chain<'b, U, V: Chain<Item=U>>(self, f: impl Fn(O) -> V + Clone + 'a) -> Parser<'a, T, V, E, impl ParseFn<T, V, E> + Captures<'b> + 'a>
        where 'a: 'b, 'b: 'a
    {
    ...
    pub fn discard<'b>(self) -> Parser<'a, T, Single<()>, E, impl ParseFn<T, Single<()>, E> + Captures<'b> + 'a>
        where 'a: 'b, 'b: 'a
    {
        self.map(|_| ())
    }
    ...
    pub fn to<'b, U: Clone + 'a>(self, to: U) -> Parser<'a, T, Single<U>, E, impl ParseFn<T, Single<U>, E> + Captures<'b> + 'a>
        where 'a: 'b, 'b: 'a
    {
        self.map(move |_| to.clone().into())
    }

    /// Create a parser that optionally parses symbols that match this parser.
    pub fn or_not<'b, V>(self) -> Parser<'a, T, Vec<V>, E, impl ParseFn<T, Vec<V>, E> + Captures<'b> + 'a>
        where O: Chain<Item=V>, 'a: 'b, 'b: 'a
    {
        Parser::generic_raw(move |tokens| {
            match self.f.parse(tokens) {
                Ok((fail, output)) => Ok((fail, output.into_iter_chain().collect())),
                Err(fail) => Ok((fail.into(), Vec::new())),
            }
        })
    }
    ...
    pub fn chained<'b>(self) -> Parser<'a, T, Single<O>, E, impl ParseFn<T, Single<O>, E> + Captures<'b> + 'a>
        where 'a: 'b, 'b: 'a
    {
        self.map(|output| output)
    }
    */

    // TODO: Rename 'also'
    /// Create a parser that parses symbols that match this parser and then another parser.
    pub fn also<'b, U>(self, other: Parser<'a, T, U, E, impl ParseFn<T, U, E>>) -> Parser<'a, T, Single<(O, U)>, E, impl ParseFn<T, Single<(O, U)>, E> + Captures<'b> + 'a>
        where U: Chain, 'a: 'b, 'b: 'a
    {
        Parser::new(AlsoFn(self.f, other.f))
    }

    /// Create a parser that parses symbols that match this parser and then another parser.
    ///
    /// Unlike `.then`, this method will chain the two parser outputs together as a chain.
    pub fn then<'b, U: 'a, V>(self, other: Parser<'a, T, U, E, impl ParseFn<T, U, E> + 'a>) -> Parser<'a, T, Vec<V>, E, impl ParseFn<T, Vec<V>, E> + Captures<'b> + 'a>
        where O: Chain<Item=V>, U: Chain<Item=V>, 'a: 'b, 'b: 'a
    {
        Parser::new(ChainFn(self.f, other.f, PhantomData))
    }

    /// Create a parser that with a flatter output than this parser.
    pub fn flatten<'b>(self) -> Parser<'a, T, Vec<<O::Item as Chain>::Item>, E, impl ParseFn<T, Vec<<O::Item as Chain>::Item>, E> + Captures<'b> + 'a>
        where O::Item: Chain, 'a: 'b, 'b: 'a
    {
        self.map_chain(|a| a.into_chain().into_iter_chain().map(|a| a.into_chain().into_iter_chain()).flatten().collect::<Vec<_>>())
    }

    fn also_chain<'b, U: Chain + 'b, G: ParseFn<T, U, E> + 'b>(self, other: Parser<'a, T, U, E, G>) -> Parser<'a, T, Single<(O, U)>, E, impl ParseFn<T, Single<(O, U)>, E> + Captures<'b> + 'a>
        where 'a: 'b, 'b: 'a
    {
    ...
    pub fn delimiter_for<'b, U: 'a>(self, other: Parser<'a, T, U, E, impl ParseFn<T, U, E>>) -> Parser<'a, T, U, E, impl ParseFn<T, U, E> + Captures<'b> + 'a>
        where U: Chain, 'a: 'b, 'b: 'a
    {
        self.also_chain(other).map_chain(|then| { let (_, b) = then.into_inner(); b })
    }
    ...
    pub fn delimited_by<'b, U: 'a>(self, other: Parser<'a, T, U, E, impl ParseFn<T, U, E>>) -> Parser<'a, T, O, E, impl ParseFn<T, O, E> + Captures<'b> + 'a>
        where U: Chain, 'a: 'b, 'b: 'a
    {
        self.also_chain(other).map_chain(|then| { let (a, _) = then.into_inner(); a })
    }

    /// Create a parser that accepts the valid input of this parser separated by the valid input of another.
    pub fn separated_by<'b, U: 'a, V>(self, other: Parser<'a, T, U, E, impl ParseFn<T, U, E>>) -> Parser<'a, T, Vec<V>, E, impl ParseFn<T, Vec<V>, E> + Captures<'b> + 'a>
        where O: Chain<Item=V>, U: Chain, 'a: 'b, 'b: 'a
    {
            ...
            for i in 0.. {
                match self.f.parse(tokens) {
                    Ok((err, output)) => {
                        ...
                        outputs.extend(output.into_iter_chain());
                    },
                    Err(err) if i == 0 => {
                        max_err = max_err.max(err);
                        break;
    ...
    pub fn padded<'b, U: Padded>(self) -> Parser<'a, T, O, E, impl ParseFn<T, O, E> + Captures<'b> + 'a>
        where U: Chain, T: Borrow<U>, 'a: 'b, 'b: 'a
    {
    ...
    pub fn or<'b, U: 'a, V>(self, other: Parser<'b, T, U, E, impl ParseFn<T, U, E>>) -> Parser<'b, T, Vec<V>, E, impl ParseFn<T, Vec<V>, E> + Captures<'b> + 'a>
        where O: Chain<Item=V>, U: Chain<Item=V>, 'a: 'b, 'b: 'a
    {
        Parser::new(OrFn(self.f, other.f, PhantomData))
    }
    ...
    pub fn or_chain_fallback<U: 'a, G: ParseFn<T, U, E>>(self, other: Parser<'a, T, U, E, G>) -> Parser<'a, T, Vec<O::Item>, E, OrChainFallbackFn<F, G, O, U>>
        where U: Chain<Item=O::Item>
    {
    ...
    pub fn repeat<'b, V>(self, repeat: impl Into<Repeat>) -> Parser<'a, T, Vec<V>, E, impl ParseFn<T, Vec<V>, E> + Captures<'b> + 'a>
        where O: Chain<Item=V>, 'a: 'b, 'b: 'a
    {
    ...
    /// Create a parser that parses symbols that match this parser and then another parser.
    pub fn collect<'b, U>(self) -> Parser<'a, T, Single<U>, E, impl ParseFn<T, Single<U>, E> + Captures<'b> + 'a>
        where U: FromIterator<<O as Chain>::Item>, 'a: 'b, 'b: 'a
    {
        self.map(|output| output.into_iter_chain().collect::<U>().into())
    }

    /// Create a parser that left-reduces this parser down into another type according to the given reduction function.
    pub fn reduce_left<'b, A, B>(self, f: impl Fn(B, A) -> A + Clone + 'a) -> Parser<'a, T, Single<A>, E, impl ParseFn<T, Single<A>, E> + Captures<'b> + 'a>
        where O: ReduceLeft<A, B>, 'a: 'b, 'b: 'a
    {
        self.map(move |output| output.reduce_left(&f).into())
    }

    /// Create a parser that right-reduces this parser down into another type according to the given reduction function.
    pub fn reduce_right<'b, A, B>(self, f: impl Fn(A, B) -> A + Clone + 'a) -> Parser<'a, T, Single<A>, E, impl ParseFn<T, Single<A>, E> + Captures<'b> + 'a>
        where O: ReduceRight<A, B>, 'a: 'b, 'b: 'a
    {
        self.map(move |output| output.reduce_right(&f).into())
    }

    /// Attempt to parse an array-like list of symbols using this parser.
    pub fn parse(&self, tokens: &[T]) -> Result<O::Item, E> {
        ...
        if let Some((idx, tok)) = tokens.next() {
            ...
            Ok(output.into_iter_chain().next().unwrap())
        }
    }
}

/*
impl<'a, T: Clone + 'a, A, E: ParseError<T> + 'a, F: ParseFn<T, Single<A>, E> + 'a> Parser<'a, T, Single<A>, E, F> {
    // TODO: Rename 'also'
    /// Create a parser that parses symbols that match this parser and then another parser.
    pub fn also<'b, B>(self, other: Parser<'a, T, Single<B>, E, impl ParseFn<T, Single<B>, E>>) -> Parser<'a, T, Single<(A, B)>, E, impl ParseFn<T, Single<(A, B)>, E> + Captures<'b> + 'a>
        where 'a: 'b, 'b: 'a
    {
        Parser::new(AlsoFn(self.f, other.f))
    }
...
impl<'a, O: Chain + 'a, E: ParseError<char> + 'a, F: ParseFn<char, O, E> + 'a> Parser<'a, char, O, E, F> {
    /// Attempt to parse a string using this parser.
    pub fn parse_str(&self, string: impl AsRef<str>) -> Result<O::Item, E> {
...
pub struct Declaration<'a, T: Clone + 'a, O: Chain + 'a, E: ParseError<T> = DefaultParseError<T>> {
    parser: Rc<RefCell<Option<Parser<'a, T, O, E>>>>,
}

impl<'a, T: Clone + 'a, O: Chain + 'a, E: ParseError<T>> Default for Declaration<'a, T, O, E> {
    fn default() -> Self {
        Self { parser: Rc::new(RefCell::new(None)) }
    }
}

impl<'a, T: Clone + 'a, O: Chain + 'a, E: ParseError<T> + 'a> Declaration<'a, T, O, E> {
...
pub fn declare<'a, T: Clone + 'a, O: Chain + 'a, E: ParseError<T> + 'a>() -> Declaration<'a, T, O, E> {
    Declaration::default()
}
...
pub fn recursive<'a, T: Clone + 'a, O: Chain + 'a, E: ParseError<T> + 'a>(f: impl FnOnce(Parser<'a, T, O, E>) -> Parser<'a, T, O, E> + 'a) -> Parser<'a, T, O, E> {
...
pub fn sym<'b, 'a, T: Clone + 'a + PartialEq, E: ParseError<T> + 'a, U: Borrow<T> + Clone + 'a>(expected: U) -> Parser<'a, T, Single<T>, E, impl ParseFn<T, Single<T>, E> + Captures<'b> + 'a>
    where 'a: 'b, 'b: 'a
{
...
pub fn not_sym<'b, 'a, T: Clone + 'a + PartialEq, E: ParseError<T> + 'a, U: Borrow<T> + Clone + 'a>(expected: U) -> Parser<'a, T, Single<T>, E, impl ParseFn<T, Single<T>, E> + Captures<'b> + 'a>
    where 'a: 'b, 'b: 'a
{
    permit(move |tok: T| &tok != expected.borrow())
}
...
pub fn one_of<'b, 'a, T: Clone + 'a + PartialEq, E: ParseError<T> + 'a, U: Borrow<T> + Clone + 'a>(expected: impl IntoIterator<Item=U>) -> Parser<'a, T, Single<T>, E, impl ParseFn<T, Single<T>, E> + Captures<'b> + 'a>
    where 'a: 'b, 'b: 'a
{
...
pub fn none_of<'b, 'a, T: Clone + 'a + PartialEq, E: ParseError<T> + 'a, U: Borrow<T> + Clone + 'a>(unexpected: impl IntoIterator<Item=U>) -> Parser<'a, T, Single<T>, E, impl ParseFn<T, Single<T>, E> + Captures<'b> + 'a>
    where 'a: 'b, 'b: 'a
{
...
pub fn permit<'b, 'a, T: Clone + 'a, E: ParseError<T> + 'a>(f: impl Fn(T) -> bool + Clone + 'a) -> Parser<'a, T, Single<T>, E, impl ParseFn<T, Single<T>, E> + Captures<'b> + 'a>
    where 'a: 'b, 'b: 'a
{
    Parser::<_, Nothing<()>, _, _>::try_map(move |tok: T| if f(tok.clone()) { Ok(tok) } else { Err(E::unexpected(tok)) })
}
...
pub fn try_map<'b, 'a, T: Clone + 'a, O: 'a, E: ParseError<T> + 'a>(f: impl Fn(T) -> Result<O, E> + Clone + 'a) -> Parser<'a, T, Single<O>, E, impl ParseFn<T, Single<O>, E> + Captures<'b> + 'a>
    where 'a: 'b, 'b: 'a
{
    Parser::<_, Nothing<()>, _, _>::try_map(f)
}

/// A parser that accepts one symbol provided it passes the given test, mapping it to another symbol in the process.
pub fn maybe_map<'b, 'a, T: Clone + 'a, O: 'a, E: ParseError<T> + 'a>(f: impl Fn(T) -> Option<O> + Clone + 'a) -> Parser<'a, T, Single<O>, E, impl ParseFn<T, Single<O>, E> + Captures<'b> + 'a>
    where 'a: 'b, 'b: 'a
{
    Parser::<_, Nothing<()>, _, _>::try_map(move |tok: T| f(tok.clone()).ok_or(E::unexpected(tok)))
}

/// A parser that accepts any single symbol.
pub fn any_sym<'b, 'a, T: Clone + 'a + PartialEq, E: ParseError<T> + 'a>() -> Parser<'a, T, Single<T>, E, impl ParseFn<T, Single<T>, E> + Captures<'b> + 'a>
    where 'a: 'b, 'b: 'a
{
    permit(|_| true)
}
...
pub fn nothing<'b, 'a, T: Clone + 'a + PartialEq, E: ParseError<T> + 'a>() -> Parser<'a, T, Single<()>, E, impl ParseFn<T, Single<()>, E> + Captures<'b> + 'a>
    where 'a: 'b, 'b: 'a
{
...
pub fn padding<'b, 'a, T: Clone + 'a, U: Padded, E: ParseError<T> + 'a>() -> Parser<'a, T, Nothing<()>, E, impl ParseFn<T, Nothing<()>, E> + Captures<'b> + 'a>
    where T: Borrow<U>, 'a: 'b, 'b: 'a
{
...
pub fn call<'a, T: Clone + 'a, O: Chain + 'a, E: ParseError<T> + 'a>(f: impl Fn() -> Parser<'a, T, O, E> + 'a) -> Parser<'a, T, O, E> {
    Parser::call(f)
}
