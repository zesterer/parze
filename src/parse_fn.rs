...
impl<T, O, U, E, F, G> ParseFn<T, (O, U), E> for ThenFn<F, G>
    where
        T: Clone,
        E: ParseError<T>,
        F: ParseFn<T, O, E>,
        G: ParseFn<T, U, E>,
{
    fn parse(&self, tokens: &mut TokenIter<T>) -> ParseResult<(O, U), E> {
...
#[derive(Clone)]
pub(crate) struct OrFn<A, B>(pub A, pub B);

impl<T, O, E, F, G> ParseFn<T, O, E> for OrFn<F, G>
    where
        T: Clone,
        E: ParseError<T>,
        F: ParseFn<T, O, E>,
...
pub struct OrChainFn<A, B, O, U>(pub A, pub B, pub PhantomData<(O, U)>);

impl<A: Clone, B: Clone, O, U> Clone for OrChainFn<A, B, O, U> {
    fn clone(&self) -> Self {
        Self(self.0.clone(), self.1.clone(), PhantomData)
    }
}

impl<T, O, U, V, E, F, G> ParseFn<T, Vec<V>, E> for OrChainFn<F, G, O, U>
    where
        T: Clone,
        O: IntoChain<Item=V>,
        U: IntoChain<Item=V>,
